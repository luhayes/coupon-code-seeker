---
slug: les-cotons-de-romane
name: Les Cotons de Romane
aliases:
  - Cotons de Romane
  - lescotonsderomane
website: https://lescotonsderomane.com
logo: /assets/merchants/les-cotons-de-romane.png
primary_category: beauty
categories:
  - beauty
  - home-living
tags:
  - reusable
  - zero waste
  - eco friendly
  - made in france
  - handmade
currency: EUR
ships_to:
  - France
  - Europe
seo:
  title: "Les Cotons de Romane Deals & Info — September 2026"
  description: "Reusable cotton pads and washable sanitary towels hand-sewn in Rennes. Free delivery over €60 in France, and the return costs to know before ordering."
  keywords:
    - les cotons de romane
    - cotons lavables avis
    - serviette hygienique lavable
    - zero dechet boutique
last_updated: 2026-09-04
verified_on: 2026-09-04
---

# Les Cotons de Romane Deals & Info

**Best offer right now:** free delivery in metropolitan France on orders over **€60**. And worth knowing first: the only official shop is **[lescotonsderomane.com](https://couponcodeseeker.com/go/les-cotons-de-romane)** — several near-identical `.shop` domains copy this brand, see the warning below.

Les Cotons de Romane makes reusable, washable everyday products — cotton pads, kitchen roll, sanitary towels and pouches — hand-sewn in Rennes, alongside a range of low-waste hygiene items.

*We may earn a commission when you buy through links on this page. It never changes the price you pay or which offers we list.*

---

## ⚠️ Buy from the official domain only

This brand is widely copied. The official shop, promoted by the brand itself from its own Instagram and TikTok accounts (**@lescotonsderomane**, ~227k and ~105k followers), is:

**`lescotonsderomane.com`**

Several look-alike stores using variations such as `lescotons.shop` and `lescotonsderomanestore.shop` reproduce the brand's name, photography and product listings. They are not the brand. Automated site-reputation scanners also return conflicting scores for this brand's name precisely because of that domain confusion — one flags it, another rates it reliable — which is a symptom of the copies rather than a judgement on the real shop.

Before ordering anywhere, check the domain in the address bar character by character. If you reached a shop through an ad or a social post rather than the brand's own linked profile, check it twice.

---

## About Les Cotons de Romane

**Romane Gautret** started the business in **2019**. She trained and worked as a paediatric nurse, and the range grew out of that: reusable alternatives to the single-use cotton, wipes and pads that households get through constantly. Everything in the sewn range is made by the brand in **Rennes**, in Brittany, rather than sourced from a supplier and relabelled — which is the actual difference between this and the large volume of generic "zero waste" sets sold on marketplaces.

The brand built its audience on TikTok and Instagram, where Romane posts as the maker rather than through an agency, and it sells wholesale to independent shops through Ankorstore as well as direct. It has also run occasional open-door sales at its Rennes workshop.

Positioning honestly: this is a small maker, not a retailer, and the economics follow from that. There is no subscription programme, no loyalty scheme and no standing discount structure — the direct price is simply the price, and it is higher than a generic set of reusable pads from a marketplace. What you are paying for is a known maker, French manufacture and fabric quality. If your priority is the lowest possible price per pad, this is not the cheapest route and the brand does not claim to be.

### What Les Cotons de Romane sells

| Line | What it is |
| --- | --- |
| Reusable cotton pads | The original product — washable make-up removal pads |
| Washable sanitary towels | Reusable period protection, sewn in-house |
| Reusable kitchen roll | Washable replacement for paper towel |
| Pouches & storage | Book pouches, wash bags and accessories |
| Low-waste hygiene | Solid soaps and shampoos, bamboo brushes and combs, reusable cotton buds, solid deodorant |
| Kitchen | Dish brushes and refillable washing-up liquid |

> Prices were not captured on this pass — check the shop.

---

## Current Les Cotons de Romane offers

| # | Offer | Type | Code |
| --- | --- | --- | --- |
| 1 | Hand-sewn in Rennes, sold direct with no reseller margin | Structural | Confirmed |
| 2 | Free delivery in metropolitan France on orders over €60 | Automatic | No code needed |
| 3 | Refunds processed within 14 days, return postage at your expense | Returns | Contact support |

### 1. Buying direct

Products are made and sold by the brand itself, so there is no reseller margin in the price. There is **no discount programme, loyalty scheme or subscription** to unlock — a smaller operation than the other merchants on this site, and priced accordingly. The brand does run occasional promotions through its own social channels and has held in-person sales at the Rennes workshop, which is where any real discount tends to appear.

### 2. Free delivery over €60

Orders over **€60** delivered within metropolitan France ship free, via **GLS**. Below that, delivery is charged at the rate shown in the cart.

One deadline worth noting because it is short: a complaint about a delivery — damage, a missing item — must be raised **by email within 14 days of receipt**, with supporting evidence such as photographs. After that the goods are treated as accepted. Open the parcel and check it promptly rather than leaving it a fortnight.

### 3. Returns — 14 days to process, and you pay the postage

Once a return is approved, the refund is **processed within 14 days** of the seller receiving it. Two costs sit with you: **return postage is at your expense**, and the **original delivery charge is not refunded**. So a change of mind on a small order can cost most of the item's value in postage — worth thinking about before ordering several sizes to compare.

Faulty goods are treated differently and properly: where a defect is demonstrated, the item is refunded or replaced **at the seller's expense**.

A practical note the policy does not spell out: reusable hygiene products are commonly excluded from change-of-mind returns once opened, for obvious reasons. If you are unsure about absorbency or sizing on washable sanitary towels, ask before ordering rather than after.

---

## How to order

1. Confirm you are on **[lescotonsderomane.com](https://couponcodeseeker.com/go/les-cotons-de-romane)** — not a `.shop` look-alike.
2. Add your items to the cart.
3. Enter any code from the brand's own social channels in the discount field at checkout.
4. Push the order over **€60** if you are close to it — that is the free-delivery line in metropolitan France.
5. **Check the parcel within 14 days.** Delivery complaints must be raised by email inside that window, with evidence.
6. If you are buying hygiene products and unsure about absorbency or sizing, ask before ordering — return postage is at your expense and opened hygiene items are commonly non-returnable.

---

## Store information at a glance

| | |
| --- | --- |
| **Official website** | [lescotonsderomane.com](https://couponcodeseeker.com/go/les-cotons-de-romane) — the only official shop |
| **Founded** | 2019 |
| **Founder** | Romane Gautret, formerly a paediatric nurse |
| **Based in** | Rennes, Brittany, France |
| **Category** | Reusable and washable hygiene, home and kitchen products |
| **Manufacture** | Sewn range made in-house in Rennes |
| **Delivery** | GLS; free over €60 in metropolitan France |
| **Returns** | Refund within 14 days of receipt; return postage paid by you |
| **Also sold** | Wholesale to independent retailers via Ankorstore |
| **Contact** | lescotonsderomane@gmail.com |

---

## Frequently asked questions

**Is lescotonsderomane.com the real site?**
Yes. It is the domain the brand promotes from its own Instagram and TikTok accounts. Several `.shop` look-alikes copy the brand's name and photography and are not connected to it.

**Why do some website-checking tools flag this brand?**
Because of the copies. Automated reputation scanners return conflicting scores for this brand name — one flags it, another rates it reliable — which reflects the number of imitation domains rather than the official shop.

**Does Les Cotons de Romane have a discount code?**
There is no standing discount programme. Occasional promotions are announced through the brand's own social channels, which is the only place we would trust a code for it to come from.

**Can I return washable sanitary towels?**
Approved returns are refunded within 14 days, but **you pay the return postage** and the original delivery charge is not refunded. Reusable hygiene products are also commonly excluded from change-of-mind returns once opened, so ask before ordering if you are unsure. Genuinely faulty items are refunded or replaced at the seller's expense.

**Where are the products made?**
The sewn range is made by the brand in Rennes, France.
